...
- [x] Configurer OxLint (typescript + react + hooks + reactNative + imports)
...
| ~~3~~ | ~~Accueil~~ | ~~Declencher l'animation bento grid apres 200-300px de scroll~~ | ~~Done — ajoute `SCROLL_START_OFFSET = 250` aux interpolations Reanimated~~ |
| ~~4~~ | ~~Produit + Accueil~~ | ~~Fixer les fallbacks des boutons hero (partage + retour)~~ | ~~Done — `SUPPORTS_TOOLBAR` constant, `headerTintColor` blanc sur produit, `headerRight` fallback Ionicons sur produit + accueil~~ |
| ~~5~~ | ~~Calendrier~~ | ~~Sticker la legende en haut + retirer "mois courant"~~ | ~~Done — legende deplacee au-dessus de la FlashList (sticky sous toolbar), item "mois courant" retire, design compact inline~~ |
...
| ~~7~~ | ~~Calendrier + Recherche~~ | ~~Espacement vertical asymetrique dans les items de liste~~ | ~~Done — `Link asChild` + `Pressable` (elimine le wrapper Link), `py-3.5` pour un espacement plus genereux~~ |
...
| ~~9~~ | ~~Recherche + Calendrier~~ | ~~Impossible de dismiss le clavier sans valider~~ | ~~Done — `keyboardDismissMode="on-drag"` sur FlashList (dismiss au scroll) + `Keyboard.dismiss()` au tap sur l'état vide~~ |
