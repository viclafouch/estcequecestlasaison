...
```javascript
// metro.config.js (Expo SDK < 54 only — default since SDK 54)
...
> **Note:** Since Expo SDK 54, `experimentalImportSupport` is enabled by default.
> You do NOT need to configure it in `metro.config.js` for SDK 54+.
> The step below is only needed for SDK 52-53.

```javascript
// metro.config.js (SDK 52-53 only — default since SDK 54)
