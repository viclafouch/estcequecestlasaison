...
Then configure via `babel-preset-expo` options in `babel.config.cjs`:
...
> **Note:** If your `package.json` has `"type": "module"`, the config file MUST
> use `.cjs` extension (not `.js`) to force CommonJS. Otherwise `module.exports`
> is silently ignored.

**SDK 52-53** (legacy approach):
...
```json
// app.json (SDK 52-53 only — do NOT use for SDK 54+)
{
  "expo": {
    "experiments": {
      "reactCompiler": true
