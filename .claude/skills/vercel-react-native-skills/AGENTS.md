...
> requires `estimatedItemSize`. LegendList still uses `estimatedItemSize`.
...
**Use `borderCurve: 'continuous'` with `borderRadius` on Views:**

> **Note:** `borderCurve` only exists on `ViewStyle`, not on `ImageStyle`.
> It does not apply to `expo-image` or `react-native` `Image` components.
> Only use it on `View`, `Pressable`, and other view-based components.
